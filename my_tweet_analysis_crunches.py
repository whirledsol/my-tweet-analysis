...
class my_tweet_analysis_crunches:
    def crunch_timeseries_counts(tweets,username):
        '''
        shows daily counts over time
        '''
        
        dates = [tweet_getDate(t, dateOnly=True) for t in tweets]
        unique, counts = numpy.unique(dates, return_counts=True)
        plot_timeseries(unique,counts,kind='bar',title=f"@{username}'s Tweet Count Per Day", yLabel='Tweet Count')
